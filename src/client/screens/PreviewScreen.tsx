...
export function PreviewScreen() {
  ...
  const previewLayout = currentPhotoLayout ?? DEFAULT_PHOTO_LAYOUT;
  const previewAspectRatio = getAspectRatioString(previewLayout);
  ...
  const validate = async () => {
    ...
    try {
      ...
      let finalLayout = previewLayout;
      try {
        const { config, layout } = await loadPrimaryTemplateSnapshot();
        finalLayout = layout;
        if (config) {
      ...
      setCurrentPhoto(finalDataUrl, r.filepath, r.share_url, finalLayout);
          ...
          <div className="flex items-center justify-center min-w-0">
            {isMulti ? (
              ...
              <motion.div
                ...
                transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                className="relative"
                style={{ aspectRatio: previewAspectRatio, maxHeight: '60vh' }}
              >
                <div
